...
class ImagePainter extends StatefulWidget {
  const ImagePainter._(
      ...
      this.onTapEnd,
      this.onTapStart,
      this.customFunctionClose,
      this.customFunctionBack})
      : super(key: key);
  ...
  factory ImagePainter.file(
    ...
    double? width,
    bool? scalable,
    Function? customFunctionClose,
    Function? customFunctionBack,
    Function(Offset)? onTapEnd,
    Function(Offset)? onTapStart,
    Widget? placeholderWidget,
    List<Color>? colors,
    Widget? brushIcon,
    Widget? undoIcon,
    ...
    return ImagePainter._(
      ...
      customFunctionBack: customFunctionBack,
      customFunctionClose: customFunctionClose,
      width: width,
      placeHolder: placeholderWidget,
      colors: colors,
      isScalable: scalable ?? false,
  ...
  final Function(Offset)? onTapEnd;

  final Function(Offset)? onTapStart;

  final Function? customFunctionClose;

  final Function? customFunctionBack;
...
class ImagePainterState extends State<ImagePainter> {
                ...
                child: ValueListenableBuilder<Controller>(
                  ...
                  builder: (_, controller, __) {
                    return GestureDetector(
                      onTapDown: (details) {
                        if (widget.onTapStart != null && !_clickDragging) {
                          widget.onTapStart!(details.localPosition);
                          setState(() {
                            _clickDragging = true;
                          });
                          return;
                        }
                        if (widget.onTapEnd != null && _clickDragging) {
                          widget.onTapEnd!(details.localPosition);
                          _scaleEndGesture2(controller);
                          setState(() {
                            _clickDragging = false;
                          });
                        }
                      },
                      child: MouseRegion(
                        onHover: (details) {
                          if (_clickDragging) {
                            _scaleUpdateGesture2(details.localPosition, controller);
                          }
                        },
                        child: CustomPaint(
                          ...
                          painter: DrawImage(
                            ...
                            isDragging: _inDrag,
                            update: UpdatePoints(
                                start: _start, end: _end, painter: _painter, mode: controller.mode),
                          ),
                        ),
                      ),
                    );
  ...
  ///Fires while user is interacting with the screen to record painting.
  void _scaleUpdateGesture2(Offset onUpdate, Controller ctrl) {
    setState(
      () {
        _inDrag = true;
        _start ??= onUpdate;
  ...
  void _scaleEndGesture2(Controller controller) {
    setState(() {
      _inDrag = false;
      if (_start != null && _end != null && (controller.mode == PaintMode.freeStyle)) {
        _points.add(null);
  ...
  void _openTextDialog() {
    ...
    TextDialog.show(context, _textController, fontSize, _controller.value.color, textDelegate,
        onFinished: (context) {
      if (_textController.text != '') {
        setState(() {
          _addPaintHistory(
            PaintInfo(
                mode: PaintMode.text, text: _textController.text, painter: _painter, offset: []),
          );
        });
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          //         ),
          //         icon: Icon(
          //             paintModes(textDelegate).firstWhere((item) => item.mode == _ctrl.mode).icon,
          ...
          IconButton(
              ...
              onPressed: () {
                if (_paintHistory.isNotEmpty) {
                  ...
                  if (widget.customFunctionBack != null) {
                    widget.customFunctionBack!();
                  }
                }
              }),
          IconButton(
            ...
            onPressed: () {
              ...
              if (widget.customFunctionClose != null) {
                widget.customFunctionClose!();
              }
            },
          ),
        ],
